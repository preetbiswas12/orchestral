...
export function isAnthropicAuthEnabled(): boolean {
  // Multi-provider system: bypass OAuth when providers.json is configured
  try {
    // Dynamic import to avoid circular dependencies - use sync check
    const { isMultiProviderEnabled } = require('../providers/bridge.js')
    if (isMultiProviderEnabled()) {
      return false
    }
  } catch {
    // Provider system not yet initialized, continue with normal flow
  }
