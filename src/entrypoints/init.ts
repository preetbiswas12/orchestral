...
export const init = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    // Initialize multi-provider system (Anthropic, OpenRouter, OpenAI, Gemini, Ollama)
    // This must happen before OAuth so the provider system can override auth
    try {
      initializeMultiProvider()
      logForDebugging('[init] Multi-provider system initialized')
    } catch (error) {
      logForDebugging('[init] Multi-provider init failed:', error)
    }
    profileCheckpoint('init_after_provider_system')

    // Check for custom updates in background (non-blocking)
    try {
      const { autoCheckForUpdatesOnStartup } = await import(
        '../utils/customUpdate.js'
      )
      void autoCheckForUpdatesOnStartup()
      logForDebugging('[init] Custom update check scheduled')
    } catch (error) {
      logForDebugging('[init] Update check init failed:', error)
    }
    ...
    // NOTE: This will be skipped if multi-provider system is active (see isAnthropicAuthEnabled)
