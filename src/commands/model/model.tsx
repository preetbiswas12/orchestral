...
import { isMultiProviderEnabled } from '../../providers/bridge.js';
import { loadConfig, setDefaultModel } from '../../providers/config.js';
import { getModelsForProvider } from '../../providers/capabilities.js';
...
function SetModelAndClose({
  ...
  React.useEffect(() => {
    ...
    function setModel(modelValue: string | null): void {
      ...
      // Update provider configuration if multi-provider is enabled
      if (isMultiProviderEnabled() && modelValue) {
        const activeProvider = getActiveProvider();
        if (activeProvider) {
          try {
            setDefaultModel(activeProvider.id, modelValue);
          } catch (error) {
      ...
      // Update message for multi-provider
      if (isMultiProviderEnabled()) {
        const activeProvider = getActiveProvider();
        if (activeProvider) {
          message = `Set ${activeProvider.id} model to ${modelValue}.`;
        }
      }
...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  if (isMultiProviderEnabled()) {
    ...
    if (!args || COMMON_INFO_ARGS.includes(args)) {
      const current = cfg.providers[providerId]?.defaultModel ?? '(none)';
      const lines = ['⎿  Active provider: ' + providerId];
      lines.push('     Current model: ' + current);
      lines.push('');
      lines.push('     Available models:');
      availableModels.forEach(m => {
        lines.push(`     - ${m.id}${m.id === current ? ' (current)' : ''}`);
      });
      const nextModel = availableModels.find(m => m.id !== current)?.id || availableModels[0]?.id;
      if (nextModel) {
        lines.push('');
        lines.push(`     Use /model ${nextModel} to switch.`);
      }
      onDone(lines.join('\n'), { display: 'system' });
      return;
    }
    if (COMMON_HELP_ARGS.includes(args)) {
      onDone('⎿  Run /model to list models for the active provider, or /model <model-id> to set it.', {
        display: 'system',
      });
      return;
    }
    const requested = args === 'default' ? null : args;
    if (!requested) {
      onDone('⎿  Please choose a concrete model in provider mode (no default alias).', {
        display: 'system',
      });
      return;
    }
    const exists = availableModels.some(m => m.id === requested);
    if (!exists) {
      onDone(
        `⎿  Model '${requested}' not found for provider '${providerId}'. Run /model to list available models.`,
        { display: 'system' },
      );
      return;
    }
    setDefaultModel(providerId, requested);
    onDone(`⎿  Set ${providerId} model to ${requested}.`, { display: 'system' });
    return;
  }
