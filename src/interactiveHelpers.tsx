...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // Check if provider setup is needed FIRST (before onboarding)
  try {
    const { needsProviderSetup } = await import('./screens/ProviderSetup.js')
    if (needsProviderSetup()) {
      const { ProviderSetup } = await import('./screens/ProviderSetup.js')
      await showSetupDialog(root, done => <ProviderSetup onComplete={done} />, {
        onChangeAppState
      })
    }
  } catch (error) {
    // Provider setup failed, continue with normal flow
